...
import { createEntry, saveReport } from "./store";
...
function buildSeededEntries(userId) {
...
function ensureSeed() {
  ...
  const info = db
    ...
    .run("Amara Demo", DEMO_EMAIL, hashPassword(DEMO_PASSWORD));
  const userId = info.lastInsertRowid;

  const made = buildSeededEntries(userId);

  // Generate & store weekly reports for the three most recent completed weeks.
  ...
  for (let wk = 1; wk <= 3; wk++) {
    ...
    if (weekEntries.length > 0) {
      ...
      if (payload) {
        saveReport(userId, { weekStart, weekEnd, payload });
        seededReports++;
      }
    }
  }

  console.log(`[seed] created demo user ${userId}, ${made.length} entries, ${seededReports} reports`);
  return { seeded: true, userId, entries: made.length, reports: seededReports };
}

let didRun = false;
export function ensureSeeded() {
  // idempotent + only once per process to avoid repeated seeding work
  if (!didRun) {
    ensureSeed();
    didRun = true;
  }
