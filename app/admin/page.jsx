  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        else if (d.unlocked) { setUnlocked(true); await load(); }
      } catch {}
      setChecking(false);
    })();
  }, [load]);
  ...
  const unlock = async (e) => {
    ...
    try {
      const r = await fetch("/api/admin/auth", {
        ...
        body: JSON.stringify(
          needsSetup ? { password: pw, confirm } : { password: pw }
        ),
      });
        ...
        <div className="auth-card">
          ...
          <form onSubmit={unlock} className="card card-pad mt16">
            <h2 className="auth-title flex gap8">
              ...
              {needsSetup ? "Set your admin password" : "Admin"}
            </h2>
            <p className="auth-sub">
              {needsSetup
                ? "Choose a password to protect the admin dashboard. Pick something you'll remember \u2014 this screen only appears once."
                : "Enter your admin password to view feedback and app stats."}
            </p>

            <div className="field mt16">
              <label htmlFor="pw">{needsSetup ? "New password" : "Password"}</label>
              <input
                id="pw"
            ...
            <button className="btn btn-primary btn-block" disabled={busy || !pw}>
              {busy ? "Please wait\u2026" : needsSetup ? "Set password & continue" : "Unlock"}
            </button>

            {needsSetup && (
              <p className="muted mt16" style={{ fontSize: 12.5, lineHeight: 1.6 }}>
                Stored as a salted scrypt hash \u2014 never in plain text.
              </p>
            )}
          </form>
        </div>
