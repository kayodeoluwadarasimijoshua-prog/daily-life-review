...
export const GET = withAuth(async function GET(req, ctx) {
  ...
  const entry = await getEntry(user.id, Number(id));
...
export const PUT = withAuth(async function PUT(req, ctx) {
  const user = await requireUser();
  const { id } = await ctx.params;
  const existing = await getEntry(user.id, Number(id));
  ...
  const updated = await updateEntry(user.id, Number(id), parsed.data);
...
export const DELETE = withAuth(async function DELETE(req, ctx) {
  const user = await requireUser();
  const { id } = await ctx.params;
  const ok = await deleteEntry(user.id, Number(id));
